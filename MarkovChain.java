...
public class MarkovChain {
    ...
    public MarkovChain(List<List<String>> trainingData) {
        ...
        //creating our sequences from the training data
        for (List<String> tweet: trainingData) {
            addSequence(tweet.iterator());
        }
    }
    ...
    void addBigram(String first, String second) {
        if (first == null || second == null) {
            throw new IllegalArgumentException("One of the params is null");
        }

        if (bigramFrequencies.containsKey(first)) {
            ProbabilityDistribution<String> current = bigramFrequencies.get(first);
            current.record(second);
        } else {
            ProbabilityDistribution<String> newFreq = new ProbabilityDistribution<>();
            newFreq.record(second);
            bigramFrequencies.put(first, newFreq);
        }
    ...
    public void addSequence(Iterator<String> tweet) {
        //throwing exception if tweet is null
        if (tweet == null) {
            throw new IllegalArgumentException("Param is null");
        }

        //do nothing if tweet is empty
        if (!tweet.hasNext()) {
            return;
        }
        //saving the "first" token
        String prevToken;

        prevToken = tweet.next();
        //adding the first token to start tokens
        startTokens.record(prevToken);

        //iterating thru all the words in the tweet
        while (tweet.hasNext()) {
            String currentToken = tweet.next();
            addBigram(prevToken, currentToken);
            //updating the "first" token
            prevToken = currentToken;
        }
        //adding the last bigram
